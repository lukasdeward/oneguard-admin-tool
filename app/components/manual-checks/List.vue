...
<template>
  <div class="overflow-y-auto divide-y divide-default">
    ...
    >
      ...
      >
        <div class="flex items-center justify-between" :class="[verification.done && 'font-semibold']">
          <div class="flex items-center gap-3">
            {{ verification.customer_name }}

            <UChip v-if="verification.done" />
          </div>
        </div>
        <p class="truncate" :class="[verification.done && 'font-semibold']">
          {{ verification.manual_check_wanted ? 'Manuelle Prüfung' : 'Automatisierte Prüfung' }}
        </p>
